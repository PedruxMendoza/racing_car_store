...
<head>
	<title>Racing Car Store | Administrador y Agentes :: Ventas</title>
	<!-- Meta tag Keywords -->
      ...
      <ul>
        ...
        <div class="about py-5">
          <div class="container py-xl-5 py-lg-3">
            <h3 class="title text-capitalize text-dark text-center pb-3 mb-sm-5 mb-4">Ventas
              <span></span>
            </h3>
            <center>
            ...
            <div class="table-responsive-xl">
              <table class="table table-striped table-dark table-curved" id="ven">
                <thead>
                  <tr>
                    <td>Matricula</td>
                    <td>Marca</td>
                ...
                <tbody>
                  <?php foreach ($this->obj_venta->get_venta() as $valor) { ?>
                    <tr>
                      ...
                      <?php $id = base64_encode($valor[8]) ?>
                      <td><button type="button" class="btn btn-danger"><a a onclick="return confirm('Esta seguro de eliminar la venta del modelo <?php echo $valor[2] ?> ?');" style="color: white" href="?action=<?php echo $e ?>&id=<?php echo $id ?>">Eliminar</a></button></td>
                    </tr>
                  <?php } ?>
                </tbody>
              </table>
          ...
          <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
              <div class="modal-header" style="background-color: black">
                <h5 class="modal-title" id="exampleModalLabel" style="font-family: 'Poppins', sans-serif; color: #2471A3;">Nueva Venta</h5>
                ...
                <form action="" method="POST" autocomplete="off">
                  <div class="form-group">
                    <div class="input-group mb-2">
                      <label class="sr-only" for="inlineFormInputGroup">Coche</label>
                      <div class="input-group-prepend">
                        <div class="input-group-text">Coche</div>
                      </div>
                      <select class="form-control" name="coche" required>
                        <option value="">--Seleccione la placa del coche--</option>
                        <?php foreach ($this->obj_venta->get_vehiculos() as $vehiculos) { ?>
                          <option value="<?php echo $vehiculos[0] ?>"><?php echo $vehiculos[1] ?></option>
                        <?php } ?>
                      </select>
                    </div>
                  </div>
                  <div class="form-group">
                    <div class="input-group mb-2">
                      ...
                      <select class="form-control" name="cliente" required>
                        ...
                        <?php foreach ($this->obj_venta->get_clientes() as $cliente) { ?>
                  ...
                  <div class="form-group">
                    <div class="input-group mb-2">
                      <label class="sr-only" for="inlineFormInputGroup">Agente</label>
                      <div class="input-group-prepend">
                        <div class="input-group-text">Agente</div>
                      </div>
                      <select class="form-control" name="agente" required>
                        <option value="">--Seleccione el agente--</option>
                        <?php foreach ($this->obj_venta->get_agente() as $agente) { ?>
                          <option value="<?php echo $agente[0] ?>"><?php echo $agente[1]." ".$agente[2] ?></option>
                        <?php } ?>
                      </select>
                    </div>
                  </div>
                  <div class="form-group">
                    <div class="input-group mb-2">
                      <label class="sr-only" for="inlineFormInputGroup">Estado de Venta</label>
                      <div class="input-group-prepend">
                        <div class="input-group-text">Estado de Venta</div>
                      </div>
                      <select class="form-control" name="estado_venta" required>
                        <option value="">--Seleccione el estado de venta--</option>
                        <?php foreach ($this->obj_venta->get_estado_venta() as $estado_venta) { ?>
                          <option value="<?php echo $estado_venta[0] ?>"<?php if($estado_venta[0]==1){ echo "selected"; } ?>><?php echo $estado_venta[1] ?></option>
                        <?php } ?>
                      </select>
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="input-group mb-2">
                      ...
                      <input type="number" class="form-control" id="inlineFormInputGroup" name="total" required placeholder="Total a pagar..">
                    </div>
                  </div>
                  <center>
